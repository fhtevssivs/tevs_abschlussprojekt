...
* However, a local database for individual nodes, such as SQLite, is allowed.
...
2. **Conflict Resolution:** Since the system aims for "eventual consistency", conflicts can occur (e.g., if two clients update the same username on different nodes at the same time). The system must implement a deterministic rule for conflict resolution (e.g., "Last-Writer-Wins" based on the `uhrzeit` attribute).
...
* The submission of the work takes place via Moodle. A corresponding submission option with the deadline to be met has already been set up.

* The application should be submitted as a fully containerized solution (Docker + docker-compose.yml) including a description for execution (e.g., in a README.md). In addition, the source code must be attached.

* Furthermore, an architectural description with a block diagram is required, explaining the basic architecture. A scope of 1-2 pages, including images, is perfectly sufficient for this.

The results are to be presented in a presentation of about 10-20 minutes in the last TEVS. The presentation includes:

* Demonstration of functionality.

* Chosen architecture and communication (incl. block diagram showing the most important communication paths).

* Type of replication and fault-tolerance mechanisms.
...
## 1.4 Tools & Use of Artificial Intelligence (AI)

The use of AI-supported tools/agents for code generation, troubleshooting, concept finding, or UI design is expressly permitted.

Condition (Explainability): The team must be able at any time to explain every chosen system component, the architecture, and the exact process (especially the consistency and replication mechanisms) in detail.

Any code – whether self-written or generated by an AI – is the full responsibility of the students. If a team member cannot conclusively explain the functioning of their own project in the final presentation, this part will be evaluated as non-performed individual work.
